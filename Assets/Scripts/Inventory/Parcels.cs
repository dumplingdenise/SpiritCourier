    ...
    IEnumerator SpawnParcels()
    {
        ...
        while (spawnCount < 3)
        {
            ...
            if (!IsPositionTooClose(spawnPos, selectedParcel) && !IsSpawningInExcludedArea(spawnPos))
            {
                ...
                {
                    pickUpParcel.parcelData = new Parcels.ParcelData(spawnPos, parcelID, parcelSprite, assignedNPC);
                }
                ...
                Debug.Log($"Parcel ID: {parcelID}, Sprite: {parcelSprite} spawned at {spawnPos} is linked to NPC {assignedNPC.npcID}");
                parcelID++;
                spawnCount++;
            }
            /*}*/
